...
const normalizeCatalogRow = (product) => ({
  ...
  currentMrp: Number(product.mrp || 0),
  newPrice: Number(product.price || 0),
  newMrp: Number(product.mrp || 0),
});

const StoreUpdatePricing = () => {
  ...
  useEffect(() => {
    const fetchCatalogProducts = async () => {
      try {
        ...
        console.error("Unable to load store catalog pricing:", error);
  ...
  const handleReset = (id) => {
    setProducts((current) =>
      current.map((product) =>
        product.id === id
          ? { ...product, newPrice: product.currentPrice, newMrp: product.currentMrp }
          : product
      )
    );
  };
  ...
  const handleUpdate = async (product) => {
    const nextPrice = Number(product.newPrice);
    const nextMrp = Number(product.newMrp);

    if (!Number.isFinite(nextPrice) || !Number.isFinite(nextMrp) || nextPrice < 0 || nextMrp < 0) {
      setSnackbar({ open: true, message: "Enter valid price and MRP values.", severity: "error" });
      return;
    }

    setSavingId(product.id);
    try {
      const response = await storeWorkspaceApi.updateCatalogPricing(store.id, product.id, {
        price: nextPrice,
        mrp: nextMrp,
      });
      ...
      console.error("Unable to update store pricing:", error);
        ...
        <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 4 }}>
          <Box>
            ...
            <Typography variant="body2" color="#a3aed0" fontWeight="600">
              Batch update product pricing for {store.name}.
            </Typography>
          </Box>
        </Stack>
            ...
            <Table>
              <TableHead sx={{ bgcolor: "#fafbfc" }}>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: 900, color: "#a3aed0" }}>Current Price/Mrp</TableCell>
                  <TableCell sx={{ fontWeight: 900, color: "#a3aed0" }}>Add Price/Mrp</TableCell>
                    ...
                    <TableRow key={row.id} hover sx={{ transition: "0.2s" }}>
                      ...
                      <TableCell>
                        <Stack direction="row" spacing={2} alignItems="center">
                          <Box sx={{ width: "100px" }}>
                            <Typography variant="caption" fontWeight="800" color="#a3aed0" sx={{ mb: 0.5, display: "block" }}>
                              MRP
                            </Typography>
                            <TextField
                              size="small"
                              type="number"
                              value={row.newMrp}
                              onChange={(event) => handlePriceChange(row.id, "newMrp", event.target.value)}
                          ...
                          <Box sx={{ width: "100px" }}>
                            ...
                            <TextField
                              ...
                              type="number"
                              value={row.newPrice}
                              onChange={(event) => handlePriceChange(row.id, "newPrice", event.target.value)}
                              sx={{ "& .MuiOutlinedInput-root": { borderRadius: "10px", bgcolor: "#f8f9fc" } }}
                            />
                          </Box>
                        </Stack>
...
export default StoreUpdatePricing;

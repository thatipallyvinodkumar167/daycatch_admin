  ...
  const fetchOrders = async () => {
    try {
      ...
      const formattedData = response.data.map((user, index) => ({
        id: index + 1,
        cartId: `ORD-ONG-${4000 + user.id}`,
        cartPrice: `₹${Math.floor(Math.random() * 4000) + 500}`,
        userName: user.name,
        userPhone: user.phone.split(" ")[0],
        deliveryDate: `2024-03-${20 + (index % 5)}`,
        status: "Ongoing",
        location: index % 2 === 0 ? "In Transit" : "At Facility"
      }));
    ...
    <Box sx={{ p: 4, backgroundColor: "#f4f7fe", minHeight: "100vh" }}>
      ...
      <Paper sx={{ borderRadius: "15px", overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
        ...
        <Box sx={{ p: 3, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f1f1" }}>
          <Typography variant="h6" fontWeight="600" color="#1b2559">Live Tracking</Typography>
          <TextField
            size="small"
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ backgroundColor: "#fafbfc" }}>
                ...
                <TableCell sx={{ fontWeight: "700", color: "#a3aed0" }}>TRACKING STATUS</TableCell>
              ...
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order, index) => (
                    <TableRow key={order.id} sx={{ "&:hover": { backgroundColor: "#f9f9f9" } }}>
                      ...
                      <TableCell sx={{ color: "#475467" }}>{order.location}</TableCell>
                      <TableCell>
                        <Chip
                          label={order.status}
                          size="small"

  ...
  const fetchOrders = async () => {
    try {
      ...
      const formattedData = results.map((order, index) => ({
        ...
        cartId: order["Cart ID"] || `ORD-COM-${index}`,
        ...
        signature: order["Signature"] || "Signed",
      }));
    ...
    <Box sx={{ p: 4, backgroundColor: "#f4f7fe", minHeight: "100vh" }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="textSecondary" sx={{ mt: 1 }}>
          View all successfully delivered and completed orders.
        </Typography>
      </Box>
      ...
      <Paper sx={{ p: 3, mb: 4, borderRadius: "16px", boxShadow: "0 10px 30px rgba(0,0,0,0.05)", borderLeft: "6px solid #24d164" }}>
        <Stack direction="row" alignItems="center" spacing={2}>
            <Box sx={{ p: 1.5, borderRadius: "12px", backgroundColor: "#e6f9ed" }}>
                <TaskAltIcon sx={{ color: "#24d164" }} />
            </Box>
            <Box>
                <Typography variant="caption" color="textSecondary" fontWeight="600">COMPLETED ORDERS</Typography>
      ...
      <Paper sx={{ borderRadius: "15px", overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
        ...
        <Box sx={{ p: 3, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f1f1" }}>
          <Typography variant="h6" fontWeight="600" color="#1b2559">Delivery Success History</Typography>
          <TextField
            size="small"

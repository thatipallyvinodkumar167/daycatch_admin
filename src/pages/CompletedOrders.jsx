...
const CompletedOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await genericApi.getAll("complete orders");
      ...
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" fontWeight="700" color="#2b3674">
          ...
          <Box component="span" sx={{ fontSize: "16px", fontWeight: "400", color: "#a3aed0" }}>
            Viewing successfully delivered orders and archival records.
          </Box>
        </Typography>
      </Box>
        ...
        <Paper sx={{ flex: 1, p: 3, borderRadius: "16px", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
            <Stack direction="row" alignItems="center" spacing={2}>
                <Box sx={{ p: 1.5, borderRadius: "12px", backgroundColor: "#e6f9ed" }}>
                    <CheckCircleIcon sx={{ color: "#24d164" }} />
                </Box>
                <Box>
                    <Typography variant="caption" color="textSecondary" fontWeight="600">DELIVERED HISTORY COUNT</Typography>
      ...
      <Paper sx={{ borderRadius: "15px", overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
        ...
        <Box sx={{ p: 3, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f1f1" }}>
          <Stack direction="row" spacing={2} alignItems="center">
            <Typography variant="h6" fontWeight="700" color="#1b2559">Deliveries Fulfillment History</Typography>
          </Stack>
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ backgroundColor: "#fafbfc" }}>
                ...
                <TableCell sx={{ fontWeight: "700", color: "#a3aed0" }}>COMPLETED ON</TableCell>
            ...
            <TableBody>
              {filteredOrders.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} align="center" sx={{ py: 6 }}>No completed order history found.</TableCell>
                </TableRow>
              ) : (
                filteredOrders.map((order, index) => (
                  <React.Fragment key={order.id}>
                    <TableRow sx={{ "&:hover": { backgroundColor: "#f9f9f9" } }}>
                      ...
                      <TableCell sx={{ color: "#24d164", fontWeight: "700" }}>{order.cartId || "N/A"}</TableCell>
                      ...
                      <TableCell align="center">
                        <Chip
                          ...
                          size="small"
                          sx={{ backgroundColor: "#e6f9ed", color: "#24d164", fontWeight: "800", fontSize: "10px", textTransform: "uppercase" }}
                        />
                      </TableCell>
                      <TableCell align="center">
                        <Button 
                      ...
                      <TableCell colSpan={8} sx={{ py: 0, border: "none" }}>
                        <Collapse in={expandedOrderId === order.id} timeout="auto" unmountOnExit>
                          <Box sx={{
                            ...
                            <Typography variant="caption" fontWeight="900" color="#24d164" sx={{ textTransform: "uppercase", letterSpacing: 2 }}>Archive Management</Typography>
                            <Stack direction="row" spacing={2}>
                              <Tooltip title="View Order Fulfillment Summary">
                                <Button
                                  variant="contained"
                              ...
                              <Tooltip title="Download Final Invoice">
                                <Button
                                  variant="contained"

...
const PaymentFailedOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      console.error("Error fetching payment failed orders:", error);
    }
  };
    ...
    <Box sx={{ p: 4, backgroundColor: "#f4f7fe", minHeight: "100vh" }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="textSecondary" sx={{ mt: 1 }}>
          View orders that failed during the payment stage.
        </Typography>
      </Box>
      ...
      <Paper sx={{ p: 3, mb: 4, borderRadius: "16px", boxShadow: "0 10px 30px rgba(0,0,0,0.05)", borderLeft: "6px solid #ff4d49" }}>
        <Stack direction="row" alignItems="center" spacing={2}>
            <Box sx={{ p: 1.5, borderRadius: "12px", backgroundColor: "#fff1f0" }}>
                <ErrorOutlineIcon sx={{ color: "#ff4d49" }} />
            </Box>
            <Box>
                <Typography variant="caption" color="textSecondary" fontWeight="600">PAYMENT FAILED</Typography>
      ...
      <Paper sx={{ borderRadius: "15px", overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
        ...
        <Box sx={{ p: 3, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f1f1" }}>
          <Typography variant="h6" fontWeight="600" color="#1b2559">Failed Transactions</Typography>
          <TextField
            size="small"
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ backgroundColor: "#fafbfc" }}>
                ...
                <TableCell sx={{ fontWeight: "700", color: "#a3aed0" }}>PAYMENT METHOD</TableCell>
              ...
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order, index) => (
                    <TableRow key={order.id} sx={{ "&:hover": { backgroundColor: "#f9f9f9" } }}>
                      ...
                      <TableCell sx={{ color: "#475467" }}>{order.method}</TableCell>
                      <TableCell>
                        <Chip
                          label={order.status}
                          size="small"

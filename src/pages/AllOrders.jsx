...
const AllOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const formattedData = results.map((order, index) => ({
        ...
        cartId: order["Cart ID"] || order._id || `ORDER-${index + 1}`,
        cartPrice: `Rs. ${order["Cart price"] || 0}`,
        userName: order["User"] || "Guest User",
        userPhone:
          order.Details?.phone ||
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "Completed":
        return { bg: "#e6f9ed", color: "#24d164" };
      case "Cancelled":
        return { bg: "#fff1f0", color: "#ff4d49" };
      case "Ongoing":
        return { bg: "#e0e7ff", color: "#4318ff" };
      case "Out for Delivery":
        return { bg: "#e6f9ed", color: "#24d164" };
      case "Placed":
        return { bg: "#eff6ff", color: "#2563eb" };
      default:
        return { bg: "#f4f7fe", color: "#2b3674" };
    }
  };
    ...
    <Box sx={{ p: 4, backgroundColor: "#f4f7fe", minHeight: "100vh" }}>
      ...
      >
        <Stack direction="row" alignItems="center" spacing={2}>
          <Box
            sx={{ p: 1.5, borderRadius: "12px", backgroundColor: "#fff1f0" }}
          >
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ backgroundColor: "#fafbfc" }}>
                <TableCell sx={{ fontWeight: "700", color: "#a3aed0" }}>
                  #
                </TableCell>
                <TableCell sx={{ fontWeight: "700", color: "#a3aed0" }}>
                  Cart ID
                ...
                <TableCell
                  align="right"
                  sx={{ fontWeight: "700", color: "#a3aed0", pr: 4 }}
                >
                  Details
                </TableCell>
              </TableRow>
                ...
                filteredOrders.map((order, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell sx={{ color: "#2d60ff", fontWeight: "700" }}>
                        {order.cartId}
                      </TableCell>

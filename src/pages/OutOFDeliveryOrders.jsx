...
const OutOFDeliveryOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await genericApi.getAll("out for delivery");
      ...
      const formattedData = results.map((order, index) => ({
        ...
        status: order["Status"] || order.status || "Out for Delivery",
      ...
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" fontWeight="700" color="#2b3674">
          ...
          <Box component="span" sx={{ fontSize: "16px", fontWeight: "400", color: "#a3aed0" }}>
            Monitoring orders currently being delivered.
          </Box>
        </Typography>
      </Box>
        ...
        <Paper sx={{ flex: 1, p: 3, borderRadius: "16px", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
            <Stack direction="row" alignItems="center" spacing={2}>
                <Box sx={{ p: 1.5, borderRadius: "12px", backgroundColor: "#e6f9ed" }}>
                    <LocalShippingIcon sx={{ color: "#24d164" }} />
                </Box>
                <Box>
                    <Typography variant="caption" color="textSecondary" fontWeight="600">IN TRANSIT QUEUE</Typography>
      ...
      <Paper sx={{ borderRadius: "15px", overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
        ...
        <Box sx={{ p: 3, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f1f1" }}>
          <Stack direction="row" spacing={2} alignItems="center">
            <Typography variant="h6" fontWeight="700" color="#1b2559">Fleet Activity History</Typography>
          </Stack>
          ...
          <Table>
            ...
            <TableBody>
              {filteredOrders.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} align="center" sx={{ py: 6 }}>No orders are currently out for delivery.</TableCell>
                </TableRow>
              ) : (
                      ...
                      <TableCell colSpan={8} sx={{ py: 0, border: "none" }}>
                        <Collapse in={expandedOrderId === order.id} timeout="auto" unmountOnExit>
                          <Box sx={{
                            ...
                            <Typography variant="caption" fontWeight="900" color="#24d164" sx={{ textTransform: "uppercase", letterSpacing: 2 }}>Logistics Tracking Queue</Typography>
                            <Stack direction="row" spacing={2}>
                              <Tooltip title="Track Realtime Fleet Location">
                                <Button
                                  variant="contained"

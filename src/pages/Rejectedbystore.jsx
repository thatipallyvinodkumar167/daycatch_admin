...
const Rejectedbystore = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await genericApi.getAll("Rejected orders");
      ...
      const formattedData = results.map((order, index) => ({
        ...
        status: order["Status"] || order.status || "Rejected",
      ...
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" fontWeight="700" color="#2b3674">
          ...
          <Box component="span" sx={{ fontSize: "16px", fontWeight: "400", color: "#a3aed0" }}>
            Re-assigning orders rejected by individual stores.
          </Box>
        </Typography>
      </Box>
      ...
      <Stack direction="row" spacing={3} sx={{ mb: 4 }}>
        <Paper sx={{ flex: 1, p: 3, borderRadius: "16px", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
            <Stack direction="row" alignItems="center" spacing={2}>
                ...
                <Box>
                    <Typography variant="caption" color="textSecondary" fontWeight="600">STORE REJECTIONS</Typography>
      ...
      <Paper sx={{ borderRadius: "15px", overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
        ...
        <Box sx={{ p: 3, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f1f1" }}>
          <Stack direction="row" spacing={2} alignItems="center">
            <Typography variant="h6" fontWeight="700" color="#1b2559">Rejected Orders Queue</Typography>
          </Stack>
          ...
          <Table>
            ...
            <TableBody>
              {filteredOrders.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} align="center" sx={{ py: 6 }}>No rejected orders awaiting reassignment.</TableCell>
                </TableRow>
              ) : (
                        ...
                        <Collapse in={expandedOrderId === order.id} timeout="auto" unmountOnExit>
                          <Box sx={{
                            ...
                            <Stack direction="row" spacing={2}>
                              <Tooltip title="Assign New Store">
                                <Button
                                  variant="contained"
                                  startIcon={<StoreIcon sx={{ fontSize: "16px !important" }} />}

...
const DeliveryBoyFeedback = () => {
  ...
  const fetchFeedback = useCallback(async () => {
    setLoading(true);
    try {
      const response = await genericApi.getAll("deliveryboyfeedback");
      const results = response.results || response.data?.results || response.data || [];
      
      const formattedData = results.map((item, index) => ({
        id: item._id || index,
        user: item.deliveryBoyName || item.boyName || item.user || item.email || "Unknown Rider",
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={4} align="center" sx={{ py: 10 }}>
                    <Typography color="#a3aed0" fontWeight="600">No active reports from the fleet.</Typography>
                  </TableCell>
                </TableRow>
              ) : (
                filteredFeedback.map((item, index) => (
                  ...
                  >
                    ...
                    <TableCell sx={{ color: "#a3aed0", fontWeight: "600", pr: 4 }}>
                      {item.date ? new Date(item.date).toLocaleString() : "Live-Ground"}
                    </TableCell>
                  </TableRow>
                ))
              )}
...
export default DeliveryBoyFeedback;

...
const StoreNotifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await genericApi.getAll("store_notifications");
      const results = response.data.results || response.data || [];
      
      const formattedData = results.map((item, index) => ({
        id: item._id || index,
        title: item.title?.slice(0, 30) || "Admin Update",
        image: item.image || item.logo || "https://via.placeholder.com/50",
        store: item.store || item.selectStores || "Store " + (index + 1),
            ...
            <TableBody>
              {filtered.map((item, index) => (
                <TableRow key={item.id} sx={{ "&:hover": { backgroundColor: "#f9f9f9" } }}>
                  ...
                  <TableCell sx={{ fontWeight: "600", color: "#2b3674" }}>{item.store}</TableCell>

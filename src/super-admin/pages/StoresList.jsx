...
const StoresList = () => {
  ...
  const fetchStores = async () => {
    ...
    try {
      ...
      const isScopedSubAdmin =
        Boolean(token) && String(role || "").toLowerCase() !== "super admin";

      const response = isScopedSubAdmin
        ? await api.get("/stores")
        : await genericApi.getAll("storeList");

      const results =
        response.data?.data || response.data?.results || response.data || [];

      const normalizedList = results.map((store, index) => ({
        id: store._id || store.id || index,
        name:
          store.store_name ||
          store["Store Name"] ||
          store.name ||
        ...
        status:
          ...
          String(store.status).toLowerCase() === "approved"
            ? "Active"
            : "Inactive",
        logo:
          store.store_photo ||
          ...
          `https://ui-avatars.com/api/?name=${encodeURIComponent(
            store.store_name || store["Store Name"] || store.name || "S"
  ...
  const openStorePage = (storeId) => {
    if (!storeId) return;
    window.open(
      `/stores/details/${encodeURIComponent(storeId)}/dashboard`,
      "_blank"
    );
  };
  ...
  return (
    <Box sx={{ p: 4, backgroundColor: "#f4f7fe", minHeight: "100vh" }}>
      ...
      <Stack direction="row" spacing={3} sx={{ mb: 4 }}>
        ...
        >
          <Avatar
            sx={{ bgcolor: "#eef2ff", color: "#4318ff", width: 56, height: 56 }}
          >
        ...
        >
          <Avatar
            sx={{ bgcolor: "#e6f9ed", color: "#24d164", width: 56, height: 56 }}
          >
      ...
      >
        {loading && (
          <LinearProgress
            sx={{ "& .MuiLinearProgress-bar": { bgcolor: "#4318ff" } }}
          />
        )}
                ...
                filteredStores.map((item, index) => {
                  ...
                  return (
                    <React.Fragment key={item.id}>
                      ...
                      >
                        <TableCell
                          sx={{ color: "#a3aed0", fontWeight: "800", pl: 4 }}
                        >
                          <IconButton
                            size="small"
                        ...
                        >
                          <Box
                            onClick={() =>
                              navigate(`/all-orders?storeId=${item.id}`)
                            }
                            sx={{
                              display: "inline-flex",
                              alignItems: "center",
                              ...
                              <Grid container spacing={4}>
                                <Grid item xs={12} md={4}>
                                  ...
                                  <Typography variant="body2" color="#a3aed0">
                                    This store is part of your partner network.
                                    You can manage their settings, earnings, and
                                    performance metrics from the detailed
                                    dashboard.
                                  </Typography>
                                </Grid>

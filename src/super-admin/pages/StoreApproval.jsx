...
const formatStoreListRequest = (store, index) => {
  const storeName = store.store_name || store["Store Name"] || store.name || "Unknown Entity";

  return {
    id: store._id || store.id || `store-list-${index}`,
    sourceCollection: "storelist",
    storeWorkspaceId: store._id || store.id || "",
    linkedStoreId: store._id || store.id || "",
    storeName,
    city: store.city || store.City || "Registry Not Set",
    mobile: store.mobile || store.Mobile || store.phone || "N/A",
    email: store.email || store.Email || "N/A",
    adminShare: store.admin_share || store["admin share"] || store["Admin Share"] || "0%",
    ownerName:
      store.owner_name ||
      store.employee_name ||
      store["Employee Name"] ||
    ...
    logo: buildLogo(storeName, store.profile_pic || store["Profile Pic"] || store.logo),
    raw: store
  };
};
...
const formatStoreApprovalRequest = (store, index, linkedStoreId = "") => {
  ...
  return {
    id: store._id || store.id || `store-approval-${index}`,
    sourceCollection: "storeapproval",
    storeWorkspaceId: linkedStoreId || store._id || store.id || "",
    linkedStoreId: linkedStoreId || store._id || store.id || "",
    storeName,
    city: store.city || store.City || "Registry Not Set",
    mobile: store.mobile || store.Mobile || store.phone || "N/A",
    email: store.email || store.Email || "N/A",
    ...
    if (window.confirm("Approve this store to start selling on the platform?")) {
      try {
        ...
        if (store.sourceCollection === "storeapproval") {
          await genericApi.update("storeapproval", store.id, approvedPayload);
        } else {
          await genericApi.update("storeList", store.id, approvedPayload);
        }
        ...
        >
          <Box>
            ...
            <Typography variant="caption" color="textSecondary">
              {filteredStores.length} requests
            </Typography>
          </Box>
                        ...
                        <Tooltip title="Approve Store">
                          <IconButton
                            ...
                            sx={{
                              ...
                              borderRadius: "10px",
                              "&:hover": { bgcolor: "rgba(0, 210, 106, 0.1)" }
                            }}
                          >
                        ...
                        <Tooltip title="Reject Store">
                          <IconButton
                            ...
                            sx={{
                              ...
                              borderRadius: "10px",
                              "&:hover": { bgcolor: "rgba(255, 77, 73, 0.1)" }
                            }}
                          >
